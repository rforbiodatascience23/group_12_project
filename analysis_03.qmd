...
## Reproducing Figure 4b, showing the top 20 number of disease-outbreaks
...
Fig4b_reproduced <- outwdi |> 
  count(Disease) |> 
  arrange(desc(n)) |> 
  slice_head(n = 20) |>  #keeps the 20 most frequent diseases
  inner_join(outwdi, by = "Disease")  |> 
  mutate(Disease = Disease |> fct_infreq())  |>  #the fct_infreq orders the Disease by frequency
  ggplot(mapping = aes(x = Disease)) +
  geom_bar()+
  labs(y = "Total frequency of outbreaks \n (1996 - 2021)", 
       x = "Disease") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) + #wraps text there is over 30 characters
  theme_minimal() +
