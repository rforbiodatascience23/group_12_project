...
geo_outbreaks_wide <- final_outbreaks |>
  count(Country, iso3, icd104c, Continent, 
        Longitude, Latitude) |>
  pivot_wider(
...
ggsave(fig4a_reproduced, 
       filename = "Results/fig4a_reproduced.png", dpi = 300)
```
...
ggsave(fig3_reproduced, 
       filename = "Results/fig3_reproduced.png", dpi = 300)
```
...
```{r}
# Plot the barplot
fig4c <-ggplot(outbreaks_yearly, aes(x = Year, y = Freq)) +
...
ggsave(fig4c_reproduced, 
       filename = "Results/fig4c_reproduced.png", dpi = 300)
```
