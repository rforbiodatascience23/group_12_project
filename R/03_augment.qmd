...
geo_outbreaks_wide <- geo_outbreaks |>
  count(Country, iso3, icd104c, Continent, Coordinates) |>
  pivot_wider(
    id_cols = c("Country", "iso3","Continent", "Coordinates"),
    names_from = icd104c,
    values_from = n,
    values_fill = 0
  )
...
geo_outbreaks_wide <- geo_outbreaks_wide |>
  mutate(Freq = rowSums(select(geo_outbreaks_wide, where(is.numeric))))
