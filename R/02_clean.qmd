...
geo_outbreaks <- worldboundaries |> 
  left_join(drop_na(total_outbreaks), by = "iso3") ## here I replaced the na.omit with drop_na to be in tidyverse (Arlinda)
```
...
```{r}
geo_outbreaks <- drop_na(geo_outbreaks) #same here (Arlinda)
```
