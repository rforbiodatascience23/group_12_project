...
if ( !file.exists(str_c(dir_name, filename2))) {
  ...
  wdi <- read.csv("~/projects/group_12_project/_raw/WDICountry.csv")
}
...
wdi_clean <- wdi |> 
  select(Country.Code, Income.Group, Short.Name, Table.Name, Long.Name) |> 
  rename(iso3 = Country.Code) |> 
  mutate(iso3 = ifelse(iso3 == "XKX", "XXK", iso3)) |> #Kosovo is named to seperate iso3 names in the files, as they do not have a universially accepted iso3 code. 
  filter(Income.Group !="")
